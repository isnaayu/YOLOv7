...
from utils.plots5 import colors, plot_one_box, plot_one_box_PIL
...
class DetThread(QThread):
    ...
    def __init__(self):
        ...
        self.weights = './yolov7e6e.pt'           # 设置权重
        self.current_weight = './yolov7e6e.pt'    # 当前权重
        self.source = '0'                       # 视频源
...
class MainWindow(QMainWindow, Ui_mainWindow):
    ...
    def load_rtsp(self, ip):
        try:
            self.stop()
            MessageBox(
                self.closeButton, title='petunjuk', text='Harap tunggu, streaming video rtsp sedang dimuat', time=1000, auto=True).exec_()
            self.det_thread.source = ip
            new_config = {"ip": ip}
            new_json = json.dumps(new_config, ensure_ascii=False, indent=2)
            with open('config/ip.json', 'w', encoding='utf-8') as f:
                f.write(new_json)
            self.statistic_msg('Memuat rtsp：{}'.format(ip))
    ...
    def chose_cam(self):
        try:
            ...
            MessageBox(
                self.closeButton, title='petunjuk', text='Harap tunggu, mendeteksi perangkat kamera', time=2000, auto=True).exec_()
            # 自动检测本机有哪些摄像头
            ...
            if action:
                ...
                self.statistic_msg('Memuat Kamera：{}'.format(action.text()))
        except Exception as e:
            self.statistic_msg('%s' % e)
    ...
    def change_model(self, x):
        ...
        self.statistic_msg('Model Beralih ke%s' % x)
    ...
    def open_file(self):
        ...
        if not os.path.exists(open_fold):
            open_fold = os.getcwd()
        name, _ = QFileDialog.getOpenFileName(self, 'Pilih Video atau Gambar', open_fold, "Pic File(*.mp4 *.mkv *.avi *.flv "
                                                                          "*.jpg *.png)")
        if name:
            self.det_thread.source = name
            self.statistic_msg('Memuat File：{}'.format(os.path.basename(name)))
    ...
    def run_or_continue(self):
        ...
        if self.runButton.isChecked():
            ...
            source = 'Perlengkapan Kamera' if source.isnumeric() else source
            self.statistic_msg('Pengujian >> Model：{}，Dokumen：{}'.
                               format(os.path.basename(self.det_thread.weights),
                                      source))
        else:
            self.det_thread.is_continue = False
            self.statistic_msg('Berhenti Sebentar')
    ...
    def closeEvent(self, event):
        ...
        with open(config_file, 'w', encoding='utf-8') as f:
            f.write(config_json)
        MessageBox(
            self.closeButton, title='petunjuk', text='Harap Tunggu, Program ditutup ...', time=2000, auto=True).exec_()
        sys.exit(0)
